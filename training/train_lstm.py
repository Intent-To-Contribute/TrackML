...
transformation_idx = input("1. dbscan_trans\n2. spherical\n3. cylindrical\n4. normalize\n5. standard\n6. identity\nChoose which transform to train: ")
transformation_idx = int(transformation_idx)-1
num_epochs = input("Input how many epochs you want to train: ")
num_epochs = int(num_epochs)
cache_prefix = "../../Data/train_full/cache/"
for event_id, hits, cells, particles, truth in load_dataset('../../Data/train_full/'):
    print("-"*15, "processing event #", event_id, "-"*15)
    cache_event_prefix = cache_prefix + str(event_id)
    ...
    if not os.path.exists(cache_event_prefix + "true_tracks.npy") or not os.path.exists(cache_event_prefix + "hit_tracks.npy"):
        ...
        for i in range(len(sorted_truth)):
            ...
            if truth[1] != current_pid and current_pid != -1:
                ...
                track.append(truth)
        np.save(cache_event_prefix + "true_tracks.npy", true_tracks)
        np.save(cache_event_prefix + "hit_tracks.npy", hit_tracks)
    else:
        true_tracks = np.load(cache_event_prefix + "true_tracks.npy")
        hit_tracks = np.load(cache_event_prefix + "hit_tracks.npy")
    ...
    #if not os.path.exists(cache_event_prefix + "true_X.npy") or not os.path.exists(cache_event_prefix + "true_Y.npy"):
    if False and not os.path.exists(cache_event_prefix + "true_X.npy") or not os.path.exists(cache_event_prefix + "true_Y.npy"):
        true_X = []
        true_Y = []
        for track in true_tracks:
            #for i in range(1, len(track)-1):
            for i in range(3, len(track)-1):
                x_hit = np.zeros((0, 3))
                for z in range(i):
        ...
        np.save(cache_event_prefix + "true_X.npy", true_X)
        np.save(cache_event_prefix + "true_Y.npy", true_Y)
    else:
        true_X = np.load(cache_event_prefix + "true_X.npy")
        true_Y = np.load(cache_event_prefix + "true_Y.npy")
    ...
    if (trainTrue):
        ...
        #idx = input("1. dbscan_trans\n2. spherical\n3. cylindrical\n4. normalize\n5. standard\n6. identity\nChoose which transform to train: ")
        #idx = int(idx)-1
        #num_epochs = input("Input how many epochs you want to train: ")
        #num_epochs = int(num_epochs)
        start_end_indices = list(transformation_indices.values())
        seq_len = 1 + start_end_indices[transformation_idx][1] - start_end_indices[transformation_idx][0]
        ...
        model.save(transformations[transformation_idx] + ".keras")
